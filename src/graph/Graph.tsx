...
import {Domain, DomainCreate, DomainUpdate, GraphDelta, Node, Relation} from "../api/model/Model";
...
export const Graph = () => {
  ...
  function updateState(graphDelta: GraphDelta) {
    setDomains(domains.filter(d => !graphDelta.removedDomains.some(id => id === d.id)))
    updateNodes(graphDelta);
    updateRelations(graphDelta);
  }

  function updateNodes(graphDelta: GraphDelta) {
    let d3Nodes = nodes.filter(n => !graphDelta.removedNodes.some(id => id === n.node.id));
    graphDelta.changedNodes?.forEach(cn => {
      if (d3Nodes.some(n => n.node.id === cn.id)) {
        d3Nodes.filter(n => n.node.id === cn.id)
        .forEach(n => n.node = cn);
      } else {
  ...
  function updateRelations(graphDelta: GraphDelta) {
    let d3Relations = relations.filter(r => !graphDelta.removedRelations.some(id => id === r.relation.id));
    graphDelta.changedRelations.forEach(cr => {
      if (d3Relations.some(n => n.relation.id === cr.id)) {
        d3Relations.filter(n => n.relation.id === cr.id)
  ...
  useEffect(() => {
    ...
    function relWidth(rel: D3Relation): number {
      ...
      if (isSelected) {
        return 18
      } else if (rel.relation.domainIds.some(id => id === selectedDomain?.id)) {
        return 12
      }
      return 6
    }
    ...
    function drawSelectionIndicator(svg: d3.Selection<null, unknown, null, undefined>) {
    ...
    function drawRelations(svg: d3.Selection<null, unknown, null, undefined>, relations: D3Relation[]) {
    ...
    function relationMouseEvents(simulation: d3.Simulation<any, any>, relation: any) {
    ...
    function relationDrawEvents(simulation: d3.Simulation<any, any>, selection: any) {
      let selectionDrag = d3.drag()
      .on('start', (e, d: any) => {
        simulation.restart()
      })
      .on('drag', (e: any, d: any) => {
    ...
    function buildSimulation(relations: D3Relation[], tick: () => void) {
      ...
      .force("link", d3.forceLink<D3Node, D3Relation>(relations)
          .id(d => d.node.id)
          .distance(100)
          .strength(0.9)
      )
    ...
    if (d3Container.current && nodes) {
      ...
      rels.forEach(r => {
        r.incomingRelationsCount = rels.filter(
            r2 => r2.relation.targetId === r.relation.sourceId
      ...
      .forEach(r => {
        r.relCount = rels.filter(
            r2 => r2.relation.sourceId === r.relation.sourceId
                && r2.relation.targetId === r.relation.targetId
        ).length;

        r.relIndex = rels.filter(
            r2 => r2.relation.sourceId === r.relation.sourceId
                && r2.relation.targetId === r.relation.targetId
        ).indexOf(r);
        ...
        if (selection && selected) {
          ...
          .attr("d", d => {
            if (d.d) {
              ...
              // @ts-ignore
              return D3Helper.selectionPath(selected.x, selected.y, selected.x, selected.y)
            }
          })
        }
  ...
  function editSection() {
    if (selected?.kind === 'node') {
      return <NodeEdit
          ...
          onDelete={deleted => {
            graphService.nodeDelete(deleted.id)
            .then(graphDelta => {
              updateState(graphDelta)
              setSelected(undefined)
            })
      ...
      return <RelationEdit
          ...
          nodes={nodes.map(n => n.node)}
          onCreate={rel => {
            graphService.relationPost(rel)
            .then(graphDelta => {
              updateState(graphDelta)
              setSelected(relations.filter(r => r.relation.id === graphDelta.changedRelations[0].id)[0])
            })
          }}
          onSubmit={rel => {
            graphService.relationPut(rel)
            .then(graphDelta => {
              updateState(graphDelta)
          ...
          onDelete={(rel) => {
            graphService.relationDelete(rel.id)
            .then(graphDelta => {
              updateState(graphDelta)
              setSelected(undefined)
            })
  ...
  function buttons() {
    return <div className={classes.buttonContainer}>
      ...
      <IconButton
          className={classes.button}
          component="span"
          color="inherit" onClick={() => setUpload(true)}>
        <CloudUpload/>
      </IconButton>
      {createUpload()}
      <IconButton
