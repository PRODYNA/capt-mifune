...
export const PipelineRow = (props: { domain: Domain}) => {
  ...
  useEffect(() => {
    let sseClient = new EventSource('http://localhost:8081/graph/domain/' + props.domain.id + '/stats',);
  ...
  return (
      <TableRow key={props.domain.id} onClick={() => history.push("/pipeline/" + props.domain.id)}>
        <TableCell component="th" scope="row">
          {props.domain.name}
        </TableCell>
        <TableCell align="left">
          {valid(props.domain.modelValid)}
        </TableCell>
        <TableCell align="left">
          {valid(props.domain.mappingValid)}
        </TableCell>
        <TableCell align="left">
